/* ====================================================================== *
 * Sylva 3D - Living Green Procedural Three.js Engine
 * Procedural Moss Roots, Fur Blades, Ferns, Blooms, Polling Particles,
 * Cursor Spray, and Animated Fluttering & Perching Butterfly
 * ====================================================================== */

(function () {
  'use strict';

  function initSylvaScene() {
    const canvas = document.getElementById('footer-3d-scene');
    const container = document.getElementById('footer-3d-wrap') || (canvas ? canvas.parentElement : null);
    ...
    let nearGroup, farGroup, motes, shadowMesh, glowMesh;
    let W = 1, H = 1, DIST = 1400;
    let poleTex = null;
    let flowerTex = null;
    let scanning = false, scanT = 0, scanMax = 3000;
    const SCAN_DUR = 3.4;
    let clock = null;
    let bf = null;
    ...
    const KEY = new THREE.Vector3(-0.30, 0.92, 0.28).normalize();
    const FILL = new THREE.Vector3(0.12, -0.86, 0.50).normalize();
    const UP = new THREE.Vector3(0, 1, 0);
    const TAU = Math.PI * 2;
    const BOXW = 10;

    // Pseudo-random deterministic noise generator
    let a_rng = 0x3f9a1c7b;
    function rng() {
      a_rng |= 0;
      a_rng = (a_rng + 0x6d2b79f5) | 0;
      let t = Math.imul(a_rng ^ (a_rng >>> 15), 1 | a_rng);
    ...
    function mossCap(p, n, steep) {
      const upness = n.y + n.z * (0.10 + 0.42 * steep) - n.x * (0.05 + 0.45 * steep);
      const fray = fbm2(p.x * 2.30 + 4.4, p.z * 2.30 - p.y * 1.90) - 0.5;
      const tongue = fbm2(p.x * 0.95 + 21.0, p.z * 0.95 - p.y * 0.80) - 0.5;
      const patch = fbm2(p.x * 0.52 + 9.3, p.z * 0.52 + p.y * 0.44);
      const c = sstep(0.16, 0.70, upness + fray * 0.40 + tongue * 0.52);
    ...
    function growOffshoot(list, start, dir, len, r0, gen) {
      ...
      const bow = gen === 0 ? rand(0.10, 0.46) : rand(-0.34, 0.42);
      ...
      const pts = [start.clone(), node(0.32, bow * 0.30, kink * 0.70), node(0.68, bow * 0.85, kink * 0.24), node(1.0, bow, kink * 0.44)];
      ...
      for (let i = 0; i < kids; i++) {
        ...
        let ax = new THREE.Vector3().crossVectors(tan, UP);
    ...
    function buildNearRoot() {
      ...
      const legRw = table([0.30, 0.28, 0.26, 0.25, 0.24, 0.23, 0.22]);
      const legMoss = table([0.24, 0.24, 0.23, 0.22, 0.21, 0.20, 0.19]);
      limbs.push(makeLimb(P, [
        [0.532, 0.860,  0.20],
        [0.572, 0.700,  0.28],
        [0.612, 0.540,  0.34],
      ...
      const legR = table([0.23, 0.25, 0.27, 0.30, 0.33, 0.36, 0.40]);
      const legRm = table([0.19, 0.20, 0.21, 0.22, 0.24, 0.25, 0.26]);
      limbs.push(makeLimb(P, [
        [0.706, 0.176, -0.02],
        [0.740, 0.158,  0.02],
        [0.772, 0.245, -0.08],
    ...
    function lightUniforms(extra) {
      ...
      if (extra) for (let k in extra) if (extra.hasOwnProperty(k)) u[k] = extra[k];
      return u;
    }
    ...
    function assembleRoot(limbs, opt) {
      ...
      const shell = new THREE.Mesh(geo, barkMaterial({
      ...
      const grass = new THREE.Mesh(bg, grassMaterial({
        uniforms: uni, transparent: soft, depthWrite: true
      }));
      ...
      for (let i = 0; i < limbs.length; i++) { limbs[i].grid = limbs[i].gnrm = limbs[i].gcaps = null; }
    ...
    // Cursor spray
    const SPRAY_N = 620, SPRAY_LIFE = 1.6;
    let spray = null, sprayPos, sprayVel, sprayBirth, sprayRnd;
    let sprayHead = 0, sprayIdle = 0, sprayDirty = false;
    const sprayPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), -240);
    const sprayAt = new THREE.Vector3(), sprayLast = new THREE.Vector3(9999, 0, 0);
    ...
    // Butterfly wing geometry & shaders
    function wingGeometry(hind) {
      const NS = 30, NU = 10, pos = [], uv = [], idx = [];
      for (let i = 0; i < NS; i++) {
        const sp = i / (NS - 1);
        let lead, chord, span;
        if (!hind) {
        ...
        for (let j = 0; j < NU; j++) {
          const u = j / (NU - 1);
          const cam = 0.030 * Math.sin(Math.PI * u) * (1 - 0.35 * sp);
          pos.push(0.018 + sp * span, cam, lead - chord * u);
          uv.push(sp, u);
        }
      }
      for (let i = 0; i < NS - 1; i++) for (let j = 0; j < NU - 1; j++) {
        const a = i * NU + j, b = a + NU;
    ...
    function wingTexture() {
      ...
      function gn(x, y) {
        const ix = Math.floor(x), iy = Math.floor(y), fx = x - ix, fy = y - iy;
        const ux = fx * fx * (3 - 2 * fx), uy = fy * fy * (3 - 2 * fy);
        const g00 = h2(ix, iy), g10 = h2(ix + 1, iy), g01 = h2(ix, iy + 1), g11 = h2(ix + 1, iy + 1);
        const a = g00[0] * fx + g00[1] * fy, b = g10[0] * (fx - 1) + g10[1] * fy;
        const c = g01[0] * fx + g01[1] * (fy - 1), e = g11[0] * (fx - 1) + g11[1] * (fy - 1);
      ...
      const b255 = v => Math.max(0, Math.min(255, Math.round((v * 0.5 + 0.5) * 255)));
      for (let yi = 0; yi < N; yi++) {
        const u = yi / (N - 1);
        for (let xi = 0; xi < N; xi++) {
          const sp = xi / (N - 1), o = (yi * N + xi) * 4;
    ...
    function buildButterfly(host, limbs, uni) {
      ...
      const foreG = wingGeometry(false), hindG = wingGeometry(true);
      const foreM = wingMaterial(false, bend.fore, tex, uni), hindM = wingMaterial(true, bend.hind, tex, uni);

      const wR1 = new THREE.Mesh(foreG, foreM), wL1 = new THREE.Mesh(foreG, foreM);
      const wR2 = new THREE.Mesh(hindG, hindM), wL2 = new THREE.Mesh(hindG, hindM);
      ...
      const N = 30, R_body = 9, pos_body = [], idx_body = [];
      for (let i = 0; i <= N; i++) {
        const a = i / N;
        let r = 0.014 + 0.026 * Math.sin(Math.PI * Math.pow(a, 0.80));
        r += 0.020 * Math.exp(-Math.pow((a - 0.70) / 0.14, 2));
        r += 0.013 * Math.exp(-Math.pow((a - 0.97) / 0.05, 2));
        const z = -0.55 + a * 0.72;
        for (let j = 0; j <= R_body; j++) {
          const th = (j / R_body) * TAU;
          pos_body.push(Math.cos(th) * r, Math.sin(th) * r * 0.90, z);
        }
      }
      for (let i = 0; i < N; i++) for (let j = 0; j < R_body; j++) {
        const q = i * (R_body + 1) + j, w = q + R_body + 1;
        idx_body.push(q, w, q + 1, w, w + 1, q + 1);
      }
      const g_body = new THREE.BufferGeometry();
      g_body.setAttribute('position', new THREE.Float32BufferAttribute(pos_body, 3));
      g_body.setIndex(idx_body); g_body.computeVertexNormals();
      group.add(new THREE.Mesh(g_body, bodyMat));
      ...
      const L = limbs[0];
      const pp = new THREE.Vector3(), pn = new THREE.Vector3();
      const probeP = new THREE.Vector3(), probeN = new THREE.Vector3();
      const perchT = 0.29;
      let bestY = -2, perchTh = 0;
      for (let i = 0; i < 64; i++) {
        const th = i / 64 * TAU;
      ...
      const BOX = { x0: perch.x - 1.5, x1: perch.x + 2.1, y0: perch.y - 0.10, y1: perch.y + 1.35,
      ...
      const SPOOK_R = 0.62;
      let spook = 0;
      const toM = new THREE.Vector3(), away = new THREE.Vector3(0, 1, 0);
      const tmp = new THREE.Vector3(), prevVel = new THREE.Vector3();
      const vRight = new THREE.Vector3(), vUp = new THREE.Vector3(), vFwd = new THREE.Vector3();
      const basis = new THREE.Matrix4(), flightQ = new THREE.Quaternion(), qTmp = new THREE.Quaternion();
      ...
      return function update(dt, t) {
        ...
        let beat = 8.6 + Math.sin(t * 0.7) * 0.9 + (0.34 - (8.6 + Math.sin(t * 0.7) * 0.9)) * st.settle;
        beat *= 1 + spook * 1.15;
        st.flap += dt * beat * TAU;
        const raw = Math.sin(st.flap);
        const shaped = (raw < 0 ? -1 : 1) * Math.pow(Math.abs(raw), 0.72);
        const flyPhi = 20 + 48 * shaped, restPhi = 15 + 7 * shaped + spook * 30;
    ...
    function buildAmbient() {
      ...
      const COUNT = (window.innerWidth < 900 || (window.innerWidth * window.innerHeight) < 620000) ? 1400 : 3600;
    ...
    function layout() {
      ...
      const narrow = W < 900;
      const u = W / (narrow ? 760 : 1600);
      const A = narrow ? ARCH_N : ARCH, F = narrow ? FAR_N : FAR;
      const cover = Math.max(1, W / (narrow ? 760 : 1600));
      ...
      function place(group, box, pinFx, pinFy, z) {
        ...
        const px = (box.left + pinFx * box.w) * u - W / 2;
        const py = H / 2 - (box.top + pinFy * boxH) * u;
      ...
      const aw = A.w * u * cover, ah = aw / A.aspect;
      const cx = (A.left + 0.5 * A.w) * u - W / 2;
      const cy = H / 2 - (A.top + 0.5 * (A.w / A.aspect)) * u;
    ...
    function updateMouse(dt) {
      ...
      [[nearGroup, uMouseNear], [farGroup, uMouseFar]].forEach(pair => {
        const g = pair[0], u2 = pair[1];
        if (!g) return;
    ...
    function build() {
      ...
      const BLADES_NEAR = small ? 55000 : 150000;
      const BLADES_FAR  = small ? 16000 : 48000;
      ...
      for (let i = 0; i < 12; i++) {
        ...
        const t = rand(0.04, 0.96), th = rng() * TAU;
      ...
      nearGroup = assembleRoot(nearLimbs, {
        ...
        blades: BLADES_NEAR, ferns: small ? 22 : 40, flowers: small ? 100 : 220,
      ...
      farGroup = assembleRoot(buildFarRoot(), {
        ...
        blades: BLADES_FAR, ferns: small ? 8 : 14, flowers: small ? 35 : 75,
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initSylvaScene);
  } else {
    initSylvaScene();
  }
})();
